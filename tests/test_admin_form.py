...
class TestAdminForm(WagtailTestUtils, TestCase):
    ...
    def test_create_book(self):
        create_url = reverse('wagtailadmin_pages:add', args=[
            Book._meta.app_label, Book._meta.model_name, self.home_page.pk])
        ...
        self.assertContains(response, reverse('model_chooser', kwargs={
    ...
    def test_edit_book(self):
        ...
        book = self.home_page.add_child(instance=Book(
            title='Night Watch',
            author=Author.objects.get(name='Terry Pratchett')))
        ...
        self.assertContains(response, reverse('model_chooser', kwargs={
            'app_label': Author._meta.app_label,
            'model_name': Author._meta.model_name}))
        input = '<input id="id_author" name="author" type="hidden" value="4" required />'.format(author.pk)
